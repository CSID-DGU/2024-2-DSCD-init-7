...
#         # MultiheadAttention 인스턴스 따로 생성 (for attention matrix extraction)
#         self.multihead_attn = nn.MultiheadAttention(embed_dim=embedding_dim, num_heads=n_heads, dropout=dropout_rate)
...
#         # MultiheadAttention을 통해 attention matrix 추출
#         attn_output, attn_weights = self.multihead_attn(embedded_team, embedded_team, embedded_team)

#         # 전체 시퀀스의 평균을 구하여 점수 예측
#         avg_output = transformer_out.mean(dim=1)  # [batch_size, embedding_dim]

#         # 점수 예측
#         score = self.fc_out(avg_output)  # [batch_size, output_dim]

#         # 점수와 attention weights 반환
